...
async function getCareer(lang: string, slug: string) {
  return prisma.careerOpening.findFirst({
    where: { slug, lang, status: "OPEN", deletedAt: null },
...
export default async function CareerDetailPage({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  const { lang, slug } = await params;
  const opening = await getCareer(lang, slug);
  if (!opening) notFound();
