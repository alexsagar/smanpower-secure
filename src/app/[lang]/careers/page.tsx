...
export default async function CareersPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const openings = await prisma.careerOpening.findMany({
    where: { status: "OPEN", deletedAt: null, lang },
          ...
          {openings.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {openings.map((opening) => (
                <Link key={opening.id} href={`/${lang}/careers/${opening.slug}`} className="group block bg-white border border-brand-charcoal/10 p-8 hover:border-brand-gold hover:shadow-xl transition-all duration-300">
