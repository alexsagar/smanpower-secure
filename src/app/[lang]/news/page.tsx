...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const seo = await prisma.sEOPageMeta.findUnique({
    where: { pagePath_lang: { pagePath: "/news", lang } },
  });
...
export default async function NewsroomPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const news = await prisma.newsArticle.findMany({
    where: { status: "PUBLISHED", isPublished: true, deletedAt: null, lang },
            ...
            <div className="space-y-6">
              {news.map((item, index) => (
                <ScrollReveal key={item.id} delay={index * 0.05}>
                  <Link href={`/${lang}/news/${item.slug}`} className="group block bg-white border border-brand-charcoal/10 p-8 hover:border-brand-gold hover:shadow-xl transition-all duration-300">
