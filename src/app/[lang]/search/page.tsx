        ...
        <div className="container-wide max-w-6xl mx-auto">
          {!query ? (
            ...
            <div className="text-center py-20">
              ...
              <Link href={`/${lang}/jobs`} className="bg-brand-black text-brand-white px-8 py-4 uppercase tracking-widest text-sm font-semibold hover:bg-brand-gold hover:text-brand-black transition-colors inline-block">
                View All Jobs
              </Link>
            </div>
          ) : (
            <div className="space-y-16">
              ...
              {jobs.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {jobs.map(job => (
                      <Link href={`/${lang}/jobs/${job.slug}`} key={job.id} className="group bg-white border border-brand-charcoal/5 p-6 hover:shadow-lg transition-all hover:border-brand-gold/50 flex flex-col justify-between h-full">
                        <div>
              ...
              {stories.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {stories.map(story => (
                      <Link href={`/${lang}/success-stories/${story.slug}`} key={story.id} className="group bg-white border border-brand-charcoal/5 p-6 hover:shadow-lg transition-all hover:border-brand-gold/50 flex gap-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {facilities.map(facility => (
                        <Link href={`/${lang}/training-facilities`} key={facility.id} className="block group bg-white border border-brand-charcoal/5 p-4 hover:border-brand-gold/50 transition-colors">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {industries.map(industry => (
                        <Link href={`/${lang}/industries`} key={industry.id} className="block group bg-white border border-brand-charcoal/5 p-4 hover:border-brand-gold/50 transition-colors">
