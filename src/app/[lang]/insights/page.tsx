...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  const seo = await prisma.sEOPageMeta.findUnique({
    where: { pagePath_lang: { pagePath: "/insights", lang } },
  });
...
export default async function InsightsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const articles = await prisma.insightArticle.findMany({
    where: { status: "PUBLISHED", deletedAt: null, lang },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {articles.map((article, index) => (
                <ScrollReveal key={article.id} delay={index * 0.05}>
                  <Link href={`/${lang}/insights/${article.slug}`} className="group block bg-white border border-brand-charcoal/10 p-8 hover:border-brand-gold hover:shadow-xl transition-all duration-300 h-full">
