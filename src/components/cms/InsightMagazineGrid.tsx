...
import { ArrowUpRight, Clock, User, Tag, Sparkles, Newspaper, BookOpen } from "lucide-react";
    ...
    <div className="w-full">
      ...
      <div className="border-t-2 border-b-2 border-brand-black py-4 mb-16 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4 text-xs tracking-widest uppercase font-mono text-brand-charcoal/70">
          ...
          <span>Vol. XXVIII • Industry Intelligence & Compliance</span>
      ...
      {filteredArticles.length === 0 ? (
        ...
        <div className="space-y-20">
          {/* Broadsheet Lead Feature Article */}
          {leadArticle && (
            <article className="group bg-brand-white border border-brand-black/15 shadow-xl overflow-hidden relative">
              <div className="bg-brand-black text-brand-white px-6 py-2 flex items-center justify-between text-[11px] uppercase tracking-widest font-mono">
                ...
                <span>{leadArticle.category?.name || "Industry Insight"}</span>
              </div>
              ...
              <div className="p-8 lg:p-14 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
                    ...
                    <Image
                      src={
                        leadArticle.featuredImage
                          ? resolveImageMediaUrl(leadArticle.featuredImage, { width: 1200 })
                ...
                <div className="lg:col-span-6 flex flex-col justify-center">
                  <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-widest text-brand-muted mb-4 font-semibold">
                    ...
                    <span className="border-l border-brand-black/15 pl-3 flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-brand-gold" />
                      {calculateReadingMinutes(leadArticle.content)} min read
                    </span>
                    {leadArticle.publishDate && (
                      <span className="border-l border-brand-black/15 pl-3">
                  ...
                  {/* Summary */}
                  {leadArticle.summary && (
                    <p className="text-base md:text-lg leading-relaxed text-brand-charcoal/90 mb-8 font-sans border-l-2 border-brand-gold pl-4 italic bg-brand-off-white/70 py-3">
                      {leadArticle.summary}
                    </p>
                  )}
                  ...
                  <div className="border-t border-brand-black/10 pt-6 flex flex-wrap items-center justify-between gap-4 mt-auto">
                    <div>
                      <p className="text-xs font-bold text-brand-black uppercase tracking-wider">
                        {leadArticle.author?.name || "Seven Seas Editorial"}
                      </p>
          ...
          {/* Remaining Articles Newspaper Grid */}
