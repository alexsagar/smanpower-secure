          ...
          {stats.map((stat, i) => (
            <div key={stat.id} className="relative p-10 lg:p-14 border-b border-brand-charcoal/10 md:border-r lg:[&:nth-child(3n)]:border-r-0 group overflow-hidden flex flex-col justify-between min-h-[300px] lg:min-h-[350px]">
              ...
              <div className="relative z-10 flex justify-between items-start">
                <span className="font-brand text-brand-black/70 text-[10px] font-semibold tracking-[0.3em] uppercase group-hover:text-brand-black transition-colors duration-500">
                  {stat.label}
                </span>
                <span className="text-brand-black/60 text-[10px] font-mono">0{i + 1}</span>
              </div>
