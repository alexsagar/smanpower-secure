...
const DemandPayloadSchema = z.object({
  ...
  demandReferenceNumber: z.string().trim().max(100, "Demand Lot Number must be 100 characters or fewer").nullable().optional(),
  ...
  seoTitle: optionalText.pipe(z.string().max(120).nullable().optional()),
  metaDescription: optionalText.pipe(z.string().max(320).nullable().optional()),
  ogImageUrl: optionalText.pipe(z.string().url("Open Graph image must be a valid URL").nullable().optional()),
  canonicalUrl: optionalText.pipe(z.string().url("Canonical override must be a valid URL").nullable().optional()),
